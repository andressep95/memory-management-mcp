...
# Called by session-start.sh (Kiro hook) and Claude hook on every session.
...
[ ! -f "$ROOT/.agents/memory.state.json" ] && \
  FAILURES="$FAILURES\n- MISSING: .agents/memory.state.json → fix: echo '{\"initialized\":false,\"last_indexed_commit\":null,\"last_indexed_at\":null}' > .agents/memory.state.json"

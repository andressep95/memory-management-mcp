...
def get_file_changes(full_hash):
    ...
    for line in raw.strip().splitlines():
        ...
        if status.startswith('A'): changes['Added'].append(fpath)
        elif status.startswith('D'): changes['Removed'].append(fpath)
        else: changes['Changed'].append(fpath)
    return changes
...
def bucket_commits(commits_iterable, tag_map):
    ...
    for i, c in enumerate(commits):
        if c[0] in tag_map: tag_at[i] = (tag_map[c[0]], c[3])
    tag_indices = sorted(tag_at.keys())

    def version_for(i):
        for ti in tag_indices:
            if i <= ti: return tag_at[ti]
        return ('Unreleased', '')
